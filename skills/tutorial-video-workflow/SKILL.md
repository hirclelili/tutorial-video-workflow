...
工作台必须支持视频与逐字稿联动、文字删除与恢复、片段删除与恢复、播放位置拆分、拖动排序、字幕修改、基础视频/音频/字幕分轨、撤销、重做、自动保存、审片视频导出和剪映工程导出入口。初始视频和音频保持连续，不得按每句字幕切碎；只有真实删除或用户手动拆分才产生媒体切点。所有操作写入 `workbench/project.json`；原素材保持只读。
